...
payload = {"pageNum": 0, "pageSize": 25, "params": {"latitude": 31.233462, "longitude": 121.492156,
                                                    "currentLatitude": 34.781425, "currentLongitude": 113.663457, "P_TYPE": 2}}

with open(r"选取的地点.txt", 'r') as f:
        ...
        try:
            ...
            if (type(values).__name__ == "list"):
                for each_json in values:
                    ...
                    parkId_Dict = {"ROAD_ID": ""}
                    parkId_Dict["ROAD_ID"] = contents[5]
    ...
    graph.to_csv(r"\数据\车场信息_上海停车_总车位_type2_" +
...
# print(data)
#data = list(set(data))
berth_index = ["BERTH", "PARKING_ID", "TOTAL_BERTH"]
...
berth_graph.to_csv(r"\数据\车场信息_上海停车_总车位_berth_type2_" +
                   d.datetime.now().strftime("%Y.%m.%d-%H-%M") + ".csv", index=False)
print(berth_graph)
graph_merge = pd.merge(graph, berth_graph, how="outer")
graph_merge = graph_merge.drop_duplicates()
graph_merge.to_csv(r"\数据\车场信息_上海停车_总车位_merge_type2_" +
                   d.datetime.now().strftime("%Y.%m.%d-%H-%M") + ".csv", index=False)
...
print(parking_index_res)
parkindex = pd.DataFrame(
    data=[[parking_index_res["data"]["DATA_TIME"], parking_index_res["data"]["INDEX_NUM"]]], columns=["DATA_TIME", "INDEX_NUM"])
print("parkindex", parkindex)
